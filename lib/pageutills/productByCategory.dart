...
class _ProductByCategoryState extends State<ProductByCategory> {
  ...
  // Generate unique cache key for each category
  String getCategoryCacheKey(int categoryId) {
    return 'category_products_$categoryId';
  }

  // Load cached data from SharedPreferences
  Future<void> loadFromCache() async {
    final prefs = await SharedPreferences.getInstance();
    final key = getCategoryCacheKey(widget.categoryId);
  ...
  // Fetch latest data from API and update cache
  Future<void> fetchProducts() async {
    final url = '${AppConstant.API_URL}api/v1/product/all-product';
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['success']) {
          ...
          final prefs = await SharedPreferences.getInstance();
          final key = getCategoryCacheKey(widget.categoryId);
          prefs.setString(key, jsonEncode(results));
        } else {
          setState(() {
            isLoading = false;
          });
